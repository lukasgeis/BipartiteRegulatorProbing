...
parser.add_argument("-m", "--max", type=str, required=True)
...
data = pd.concat([
    loadAndPrepData(args.max),
    loadAndPrepData(args.sum),
    loadAndPrepData(args.cov)
])
...
palette = [palette[0], palette[0], palette[1], palette[1], palette[2], palette[2]]
dashes = [(1, 1), (3, 2), (1, 1), (3, 2), (1, 1), (3, 2)]
